...
OpenClaw 默认支持二十多个聊天渠道（WhatsApp、Telegram、Slack、飞书等等），我推荐国内用户用飞书，原因很简单：
...
> 💡 **如果你不用飞书怎么办？** 完全没问题。Telegram 也是很好的选择（海外用户推荐这个），配置方法类似。你也可以直接用 OpenClaw 自带的 WebUI（就是浏览器界面），不接聊天软件也能用，只是没有手机端随时随地用那么方便。
...
- 获取单聊、群聊消息 ⇒ `im:message` 或者 `im:message:readonly`（让机器人能看到你发的消息）
- 接收群聊消息 ⇒ `im.message.receive_v1`（让机器人能在群里收到消息）
- 接收单聊消息 ⇒ `im.message.receive_v1`（让机器人能在私聊里收到消息）
- 以应用身份发消息 ⇒ `im:message:send_as_bot`（让机器人能回复你）
...
你要是现在不确定，直接把我给你这个 JSON 复制进去导入就行，一次性开好（你不需要理解里面的 `scopes` 和 `tenant` 是什么意思，直接原样复制粘贴就行）：
...
2026 年 3 月 6 日起，OpenClaw 推出了**官方飞书插件**，安装一条命令就搞定，不用手动改 JSON 配置文件了。
